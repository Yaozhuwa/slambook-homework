...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        if (kp.pt.x>=half_patch_size && kp.pt.x<(image.cols-half_patch_size)
            && kp.pt.y>=half_patch_size && kp.pt.y<(image.rows-half_patch_size))
        {
            for(int x=-8;x<=8;x++)
            {
                for (int y=-8;y<=8;y++)
                {
                    dest_x += x*image.at<uchar>(kp.pt.y+y,kp.pt.x+x);
                    dest_y += y*image.at<uchar>(kp.pt.y+y,kp.pt.x+x);
                }
            }
            kp.angle = atan2(dest_y, dest_x)/M_PI*180;
        }
        // END YOUR CODE HERE
    }
    return;
...
// tell if a point is in image area or not.
bool pointInImage(cv::Point2f p, cv::Size imgSize){
    if (p.x<0 || p.y<0 || p.x>=imgSize.width || p.y>=imgSize.height)
        return false;
    return true;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            double vsin = sin(kp.angle/180*M_PI);
            double vcos = cos(kp.angle/180*M_PI);
            cv::Point2f pp(p.x*vcos-p.y*vsin, p.x*vsin+p.y*vcos);
            cv::Point2f qq(q.x*vcos-q.y*vsin, q.x*vsin+q.y*vcos);
            pp += kp.pt, qq += kp.pt;
            cv::Size s = image.size();
            if (!pointInImage(pp,s) || !pointInImage(qq,s)){
                d.clear();
                break;
            }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    // END YOUR CODE HERE
    for (int qureyIndex=0; qureyIndex<desc1.size(); ++qureyIndex){
        if (desc1[qureyIndex].empty())
            continue;
        int d_min = d_max;
        int min_trainIndex = -1;
        for (int trainIndex=0;trainIndex<desc2.size(); ++trainIndex){
            if (desc2[trainIndex].empty())
                continue;
            int d=0;
            for (int i=0;i<256;i++){
                if(desc1[qureyIndex][i]!=desc2[trainIndex][i])
                    d++;
            }
            if (d<d_min){
                d_min = d;
                min_trainIndex = trainIndex;
            }
        }
        if(d_min<d_max)
            matches.push_back(cv::DMatch(qureyIndex, min_trainIndex, d_min));
    }
    for (auto &m: matches) {
